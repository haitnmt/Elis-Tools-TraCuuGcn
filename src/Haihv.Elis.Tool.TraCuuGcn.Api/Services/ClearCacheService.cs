...
public class ClearCacheService(IConnectionElisData connectionElisData, ILogger logger) : BackgroundService
{
    ...
    private readonly int SecondsTimeDifference = connectionElisData.GetTimeDifferenceAsync().Result;
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                await connectionElisData.CacheRemoveAsync(TimeSpan.FromSeconds(SecondsDelay + SecondsTimeDifference), stoppingToken);
            }
            catch (Exception e)
            {
